...
<?php
$path = 'Documentos/'.$_GET['idcarpeta'].'/Factura_XML.xml';
...
echo $directorio;
$subir_archivo = $directorio.basename($_FILES['name'].'Factura_XML.xml');
echo "<div>";
if (move_uploaded_file($_FILES['subir_archivo']['tmp_name'], $subir_archivo)) {
  ...
  $con=mysqli_connect('localhost', 'sistemap_Product','zsHzOBuClJV','sistemap_Product') or die('Error en la conexion servidor');
   $idcarpeta=$_GET['idcarpeta'];
    $sql="UPDATE Enviados SET Factura_XML='Enviado' WHERE Nombre_carpeta='$idcarpeta'";
