...
$path = 'Documentos/'.$_GET['idcarpeta'].'/Factura_PDF.pdf';
...
if (file_exists($path)) {
    ...
    <?php
       ...
       ?>
        ...
        window.location.href = "actualizar2.php?idcarpeta=<?php echo $id; ?>&nombre2=<?php echo $nombre2;?>";
        </script>
<?php
   }
 else {
...
echo $directorio;
$subir_archivo = $directorio.basename($_FILES['name'].'Factura_PDF.pdf');
echo "<div>";
if (move_uploaded_file($_FILES['subir_archivo']['tmp_name'], $subir_archivo)) {
   ...
   $idcarpeta=$_GET['idcarpeta'];
    $sql="UPDATE Enviados  SET Factura_PDF='Enviado' WHERE Nombre_carpeta='$idcarpeta'";
...
$status='Pendiente';
    ...
    $sql="INSERT INTO carpetas VALUES(null,'".$_GET['nombre2']."','".$_GET['nombre']."', '".$_POST["lote"]."', 'Pendiente', 'NO')";
     ...
     if (mysqli_query($con, $sql)) {
        header("Location: pregunta.php?nombre=$hola");
    }else{
        echo"Error: ".$sql."<br>".mysqli_error($con);
    }
